...
本节进一步比较 CSER 在达到相同目标性能时的样本消耗和单步训练耗时（参考[2506.13404][hf-2506]，目标性能为准确率达到54%）。表中统计了 on-policy GRPO-ST1 与 GRPO-ST128 (w CSER) 在收敛到目标验证准确率时的训练开销，其中 rollout 样本数表示需要实时生成的新样本数量，训练样本数表示进入 learner 更新的样本总数（包括新生成样本和回放样本）。
